...
export class WoocommerceService {
  ...
  // Trae la conexion de WooCommerce del vendedor (null si no ha conectado ninguna tienda).
  ...
  // data: { profile_id, store_url, consumer_key, consumer_secret }
  connect(data: any) {
    const run = async (): Promise<any> => {
      const { data: resp, error } = await supabase.functions.invoke('woocommerce-connect', {
  ...
  // Pedidos de WooCommerce con al menos un item sin emparejar, pendientes de revision manual.
  ...
  resolvePendingOrder(pendingOrder: any, profileId: string, resolvedItems: any[]) {
    const run = async (): Promise<any> => {
      ...
      const newMappings = resolvedItems
        ...
        .map((it: any) => ({ profile_id: profileId, woocommerce_sku: it.sku, product_id: it.product_id, product_variant_id: it.product_variant_id || null }));
      ...
      const orderType = (pendingOrder.financial_status === 'processing' || pendingOrder.financial_status === 'completed') ? 'woocommerce' : 'contraentrega';
      ...
      await supabase.from('orders').update({ woocommerce_order_id: pendingOrder.woocommerce_order_id }).eq('id', orderId);
      await supabase.from('woocommerce_pending_orders').update({ resolved: true }).eq('id', pendingOrder.id);
