...
@Component({
  selector: 'app-shopify-pending',
  templateUrl: './shopify-pending.component.html',
  styleUrls: ['./shopify-pending.component.scss']
})
export class ShopifyPendingComponent implements OnInit {
  ...
  cargar() {
    ...
    this.loader = true;
    this._shopify.getPendingOrders(this.dataUser.id).subscribe(res => {
  ...
  confirmarPedido(pending: any) {
    ...
    this.saving = true;
    this._shopify.resolvePendingOrder(pending, this.dataUser.id, pending.items).subscribe(res => {
      this.saving = false;
      if (!res.success) {
